 ...
 * image, and an extra request for a 13px mark is not worth it. The lettering uses
 * currentColor so it tracks the footer's theme-derived ink; the two glyph shapes
 * keep their fixed brand grays.
 */
...
function injectStyles() {
  ...
  const brBtn = brBtnRaw
    ...
    : brSm;
  // "Powered by" strip: tint derived from the modal background via the BT.601 luma
  // test, so the footer reads as a footer on both light and dark banner themes.
  let _bfHex = String(s.bannerBg || '#FFFFFF').replace('#', '');
  if (_bfHex.length === 3) _bfHex = _bfHex[0] + _bfHex[0] + _bfHex[1] + _bfHex[1] + _bfHex[2] + _bfHex[2];
  const _bfLuma = 0.299 * (parseInt(_bfHex.substr(0, 2), 16) || 0)
    + 0.587 * (parseInt(_bfHex.substr(2, 2), 16) || 0)
    + 0.114 * (parseInt(_bfHex.substr(4, 2), 16) || 0);
  const _bfLight = _bfLuma > 128;
  const brandBg = _bfLight ? 'rgba(16,24,40,0.035)' : 'rgba(255,255,255,0.06)';
  const brandLine = _bfLight ? 'rgba(16,24,40,0.08)' : 'rgba(255,255,255,0.12)';
  const brandInk = _bfLight ? '#A2ABBA' : 'rgba(255,255,255,0.62)';
  const css = \`
...
.cb-brand-footer{box-sizing:border-box;height:44px;max-height:50px;padding:0 22px;display:flex;align-items:center;justify-content:flex-end;gap:8px;background:\${brandBg};border-top:1px solid \${brandLine};border-radius:0 0 \${br} \${br}}
.cb-brand-footer a{display:inline-flex;align-items:center;gap:7px;text-decoration:none;color:\${brandInk};border-radius:6px;padding:4px 6px;margin-right:-6px;transition:opacity .15s ease}
.cb-brand-footer a:hover{opacity:.7}
.cb-brand-credit{font-size:11px;font-weight:500;letter-spacing:.02em;line-height:1;white-space:nowrap;color:inherit}
