...
//
// REFERENCE COPY — source of truth is cdn.js (the `loader` template literal, lines 847-2835).
// This file exists for readability and navigation only.
// cdn.js does NOT import from here — edits here have no live effect.
//
// HOW THE LOADER IS ASSEMBLED IN cdn.js:
//
//   const loader = `
//   ${inlineConfig}          <- window.__CONSENT_SITE__ = { id, bannerType, apiBase, gaId, customization, ... }
//   [this IIFE — with ${translationsVar} injected inside]
//   `;
//
...
//
//   Everything else runs in the VISITOR browser at runtime.
//
// RUNTIME EXTERNAL REQUESTS (what the browser loads):
//   (no webfont — the banner renders in the system UI font stack, so nothing is
//    requested from fonts.googleapis.com or any other font host, pre- or post-consent)
...
//   - {workerOrigin}/embed/floating-logo.svg        (floating cookie button icon)
//
// SECTION MAP (approximate line numbers within this file):
//   Line ~65   Domain validation IIFE
//   Line ~79   Variable declarations (config, customization fields)
//   Line ~97   ${translationsVar} injection point
//   Line ~100  Translation helpers: R(), U(), W(), J(), Y(), X(), q(), qR(), qC()
//   Line ~220  Layout/position helpers: H(), V(), Z(), $(), G(), K(), Q()
//   Line ~340  Consent storage: ee(), te(), ne(), ae(), re(), ie(), oe(), ce()
//   Line ~500  Script detection: se(), le(), de(), pe(), be(), fe(), me(), ue(), ge(), ve()
//   Line ~640  Script blocking engine: ye(), he(), xe(), we(), ke(), Ce(), Ee(), Se(), _e()
//   Line ~760  Webflow bridge: Ie() — fires consentUpdated event (no-op unless __CB_WEBFLOW_MODE__)
//   Line ~810  Script blocking init: Oe() — installs createElement hook + MutationObserver
//   Line ~860  Blocking scanner: Ae() — scans existing scripts, blocks non-consented ones
//   Line ~905  Pending queue: Be() — releases queued scripts after consent
//   Line ~930  GA init: Le() — injects gtag/js if gaId set; sets consent default
//   Line ~980  gtag consent update: Te() — called after user action
//   Line ~1005 Styles: ze() — injects <style id="cb-styles"> + Montserrat font link
//   Line ~1070 Close button builders: Ne(), je()
//   Line ~1115 Banner HTML builder: Pe() — builds GDPR accordion OR CCPA opt-out banner
//   Line ~1600 Preferences panel builder: We()
//   Line ~1640 Event wiring: Xe() — wires all button click handlers
//   Line ~1880 Show helper: qe()
//   Line ~1890 Init: He() — main entry point called on DOMContentLoaded
//
// Last synced from cdn.js: 2026-05-29
// =============================================================================

/**
 * Returns the standard GDPR/CCPA banner loader IIFE as a string.
 * Prepend window.__CONSENT_SITE__ = {...} (inlineConfig) before embedding.
 *
 * @param {string} translationsVar  "var TRANSLATIONS = {...};" injected inside the IIFE
 * @returns {string}
 */
