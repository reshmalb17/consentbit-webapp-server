...
async function _handleCDNScript(request, env, url) {
  ...
  // Host this request came from, once authorized. Sent to the client below as
  // registeredDomain so the in-page guard does not disable a banner the server
  // has already cleared.
  ...
  if (resolvedSite.domain) {
    ...
    if (sourceHeader) {
      try {
        ...
        // Staging URL as recorded in KV by the Webflow app. Still consulted
        // because it predates Site.stagingUrl and is populated for sites the
        // column never was.
        ...
        // Unknown host on a platform we can check. A site registered with its
        // staging URL reaches here the first time its custom domain is visited:
        // ask the platform whether the domain really belongs to this site, and
        // serve straight away if it does. Confirmed hosts are written to
        // Site.customDomain, so this costs one lookup per site, not per request.
  ...
  if (customization) {
    ...
    // The banner inherits the host page's font by default — it names no family of its
    // own, so it reads as part of the site it sits on. bannerFontFamily is still stored
    // by the dashboard but deliberately not read here: we load no webfont, so a named
    // family would either resolve to nothing or silently fall back to the host page.
    //
    // The Designer app's Type → Font card can opt a site INTO the banner's own stack
    // via two additive keys in translations.config, both optional:
    //   bannerFontMode    'default' (inject the stack below) | 'inherit' (inject nothing)
    //   bannerFontEnabled '1' | '0' — fallback for callers that send only this flag
    // Absent both keys the banner inherits, which is what a site with no Font card
    // preference gets. 'inherit' on the two containers is enough on its own: every
    // child rule in the runtime's stylesheet already declares font-family:inherit.
