...
async function _runEnsureSchema(db) {
  ...
  // A signup password is hashed at request-code time and parked here until the OTP is
  // verified, so the plaintext is never written to the database and the User row gets a
  // finished PBKDF2 hash the moment the account is created.
  try {
...
export async function createEmailVerificationCode(
  db,
  { email, purpose, codeHash, name, ttlMinutes = 10, passwordHash = null } = {},
) {
  await ensureSchema(db);
  const id = crypto.randomUUID();
  const now = new Date();
  const expiresAt = new Date(now.getTime() + ttlMinutes * 60 * 1000).toISOString();
  // Preferred insert: also parks the signup password hash. Falls through to the
  // long-standing statement below on any DB that has not picked up the column yet.
  try {
    await db
      .prepare(
      ...
      .bind(id, email.trim().toLowerCase(), purpose, codeHash, name || null, passwordHash || null, expiresAt, now.toISOString())
      .run();
    return { id, expiresAt };
  } catch (e) {
...
/**
 * Password policy shared by signup and set-password. Mirrors the client-side check in
 * PasswordSection.tsx so the two cannot drift. Returns an error string, or null when
...
/**
 * Accounts created by the OTP flow carry the literal sentinel 'passwordless' (see
 * createUser's default), and older rows may be NULL or empty. None of those are hashes:
 * comparing user input against them directly would let anyone sign in to those accounts
 * by typing the sentinel, so every password path must funnel them through here.
 */
export function isPasswordSet(stored) {
  const s = typeof stored === 'string' ? stored.trim() : '';
  return s !== '' && s !== 'passwordless';
}

/**
 * Verify a plaintext password against whatever format the row happens to hold.
 * `needsRehash` flags the legacy 'client:' SHA-256 rows so a successful sign-in can
 * silently upgrade them to PBKDF2 — see handleAuthLogin.
 */
export async function verifyStoredPassword({ email, password, stored }) {
...
/**
 * Store a new PBKDF2 hash. createUser writes both passwordHash and password_hash, so
 * keep the pair in step; the fallback covers DBs that only have the camelCase column.
 */
export async function updateUserPasswordHash(db, userId, passwordHash) {
  try {
    await db
      .prepare(`UPDATE User SET passwordHash = ?1, password_hash = ?1, updatedAt = datetime('now') WHERE id = ?2`)
      .bind(passwordHash, userId)
      .run();
    return;
  } catch (e) {
    // password_hash column absent on this DB — fall through.
  }
  await db
    .prepare(`UPDATE User SET passwordHash = ?1, updatedAt = datetime('now') WHERE id = ?2`)
    .bind(passwordHash, userId)
    .run();
}
